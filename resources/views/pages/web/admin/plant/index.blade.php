...
<main>
    <div class="container">
        @yield('mobile-title')
        @yield('tab-nav-bar')
        <h5 class="secondary-text mt-4">PLANT</h5>
        <hr>
        <div class="my-2 text-end">
            <a href="{{ route('admin.plant.create') }}" class="btn btn-action btn-warna"><i class="me-3 fa-duotone fa-file-plus"></i>ADD NEW PLANT</a>
        </div>
...
<script>
    var config = {
        _token: "{{ csrf_token() }}",
        dataUrl: "{{ route('admin.plant.list') }}",
        datatableColumns: [{
                title: 'Name',
                data: 'name'
            },
...
<script>
    const baseUrl = "{{ route('admin.plant.index') }}";

    function editItem(sender) {
        let plantId = $(sender).data('id');
        window.location.href = `${baseUrl}/${plantId}/edit`;
    }
</script>
@endsection
...
<main>
    <div class="container">
        ...
        <div class="mt-3 text-end">
            <a href="{{ route('settings.break-schedule.create',[ $plant->uid ]) }}" class="btn btn-action btn-warna"><i class="me-3 fa-duotone fa-file-plus"></i>ADD NEW BREAK SCHEDULES</a>
        </div>
...
<script>
    var config = {
        _token: "{{ csrf_token() }}",
        dataUrl: "{{ route('settings.break-schedule.list',[$plant->uid]) }}",
        datatableColumns: [{
                title: 'No.',
                data: 'id',
                sortable: false,
            ...
            },
            {
                title: 'Action',
                data: 'id',
                class: 'text-center list-action col-1',
                orderable: false,
                render: function(data, type, row) {
    ...
    const baseUrl = "{{ route('settings.break-schedule.index',[$plant->uid]) }}";

...
<main>
    <div class="container pt-3">
        ...
        @yield('tab-nav-bar')
        <h5 class="secondary-text mt-4">DOWNTIME</h5>
        <hr>
        <div class="my-2 text-end">
            <a href="{{ route('settings.downtime.create',[ $plant->uid ]) }}" class="btn btn-action btn-warna"><i class="me-3 fa-duotone fa-file-plus"></i>ADD NEW DOWNTIME</a>
        </div>
...
<script>
    var config = {
        _token: "{{ csrf_token() }}",
        dataUrl: "{{ route('settings.downtime.list',[$plant->uid]) }}",
        datatableColumns: [{
                title: 'DOWNTIME TYPE',
    ...
    const baseUrl = "{{ route('settings.downtime.index',[$plant->uid]) }}";

    function editItem(sender) {
        let downtimeId = $(sender).data('id');
        window.location.href = `${baseUrl}/${downtimeId}/edit`;
    }
</script>
@endsection